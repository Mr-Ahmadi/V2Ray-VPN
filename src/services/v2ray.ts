...
export class V2RayService {
  ...
  async getSettings(): Promise<Record<string, any>> {
    ...
    const settings: Record<string, any> = {
      ...
      forceTelegramProxy: true,
      bootstrapTelegramProxy: true,
  ...
  private async waitForPortReady(port: number, timeoutMs: number = 8000): Promise<boolean> {
    const delayMs = 200;
    const maxAttempts = Math.max(1, Math.floor(timeoutMs / delayMs));
    for (let i = 0; i < maxAttempts; i++) {
      if (await this.checkProxyPort(port)) return true;
      if (i < maxAttempts - 1) {
  ...
  private async testServerLatencyByConfig(server: Server, tempKey: string): Promise<{ success: boolean; latency?: number; error?: string }> {
    try {
      ...
      return new Promise((resolve) => {
        ...
        // Wait for the temp port to be ready, then measure latency
        (async () => {
          if (settled) return;
          const portReady = await this.waitForPortReady(tempPort, 8000);
          if (settled) return;
          if (!portReady) {
            clearTimeout(timeout);
            cleanup();
            const details = startupErrorLog ? ` (${startupErrorLog})` : '';
            finalize({ success: false, error: `V2Ray did not start in time${details}` });
            return;
          }
